...
with st.sidebar:
    ...
    max_iter = st.slider("Max agent iterations", min_value=3, max_value=15, value=6)
    st.divider()
...
if run_button and company:
    ...
    tools = [
        Tool(
            ...
            Good: Apple company overview
            Good: Apple revenue 2025
...
Only use facts from your searches."""

    st.divider()
    st.subheader(f"Researching {company}...")

    try:
        with st.spinner("Agent is working — this takes 20-40 seconds..."):
